...
This means Vercel auto-detected the project as Vite instead of Next.js. Causes & fixes:
- Make sure `vercel.json` has `"framework": "nextjs"` (no `buildCommand` override)
- Make sure `package.json` `build` script is just `"next build"` (no `cp` or `vite` commands)
- Make sure there is no `vite.config.ts` or `vite.config.js` in the repo root
- In Vercel project settings → General → Framework Preset, manually set to **Next.js** if auto-detect fails
- Delete the `.vercel/` folder locally and re-link: `vercel link`

### Backend crash with `ImportError: cannot import name 'Notification' from 'app.schemas'`
Fixed in v1.0.1 — the `Notification` Pydantic schema is now defined in `app/schemas/__init__.py`. If you deployed v1.0.0, re-upload the backend folder.

### Backend logs `openrouter_key_missing` despite setting `OP_API_KEY` secret
Fixed in v1.0.1 — pydantic-settings now has `populate_by_name=True` so the `OP_API_KEY` env var is correctly loaded into `settings.op_api_key`. Verify by checking the startup log line: it should show `op_api_key_loaded=True`.
